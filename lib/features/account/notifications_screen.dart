...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  void initState() {
    super.initState();
    _future = NotificationInboxService.fetch();
  }
          ...
          builder: (context, snapshot) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                if (index == 0) {
                  final label = rows.length.toString() +
                      ' recent update' +
                      (rows.length == 1 ? '' : 's');
                  return Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(
                          label,
                          style: NileTypography.titleMedium.copyWith(
...
class _NotificationCard extends StatelessWidget {
  ...
  Future<void> _openOrder(BuildContext context) async {
    ...
    final phone = order['customer_phone_snapshot']?.toString();

    if (orderNumber == null || orderNumber.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('This order has no tracking number yet.')),
      );
      return;
    }
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: positive
                      ? NileColors.primary.withOpacity(.10)
                      : Colors.red.withOpacity(.10),
                  borderRadius: BorderRadius.circular(15),
                ),
                child: Icon(
                  NotificationInboxService.iconFor(event),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      NotificationInboxService.titleFor(event),
                    ...
                    Text(
                      NotificationInboxService.messageFor(event),
