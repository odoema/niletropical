-- 016_create_order_v2.sql — idempotent create_order matching §16 signature
--
-- FIX (2026-09-11): the prior version of this file was written against a
-- schema that never existed. It referenced orders.customer_email/notes,
-- delivery_zones.fee, order_items.product_variant_id/product_name, and
-- order_status_history.status/note — none of which are present in
-- 006_orders.sql or 008_delivery.sql. Every column reference is now
-- aligned with the base schema and payment_method is cast to the enum
-- defined in 006_orders.sql.

-- ---------------------------------------------------------------------
-- Columns create_order needs that the base 006_orders.sql did not add.
-- ---------------------------------------------------------------------
...
-- Drop any earlier broken definitions.
DROP FUNCTION IF EXISTS create_order(text, text, text, text, text, uuid, text, jsonb, text);
DROP FUNCTION IF EXISTS create_order(text, text, text, uuid, numeric, text, jsonb, jsonb, uuid, text);
